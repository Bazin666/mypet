...
class Rules(Enum):
    limit = lambda entryName, length: {'rule': li, 'args': {'entry': entryName, 'args': {'length':length}}}
    range = lambda entryName, low, up: {'rule': range, 'args': {'entry': entryName, 'args': {'low':low, 'up':up}}}
    notnull = lambda entryName: {'rule': notnull, 'args': {'entry': entryName, 'args': None}}
    regex = lambda entryName, reg: {'rule': regex, 'args': {'entry': entryName, 'args': {'reg':reg}}}
